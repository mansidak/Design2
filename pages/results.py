...
with st.sidebar:
    ...
    options2 = st.multiselect('Filter through Skill', [item[6] for item in st.session_state['FinalResults']], None)
...
for element in st.session_state['FinalResults']:
    if element[5] in options:
        ...
        with st.expander(f"{title} at {companyName}"):
            ...
            st.write(f"Skill Used:{element[6]}")
