...
"""
    ...
    if ResumePDF is not None:
        ...
        if ExperienceLevel is not None and Search:
            ...
            links4 = run_selenium1(f"-{ExperienceLevel}", f"{newSkills[1]}%2C+{newSkills[2]}", f"{undesired}", 1, resumeContent, locationpreference.replace(' ', '_'))
            st.write(links1)
            st.write(links2)
            st.write(links3)
            st.write(links4)
            ...
            #     future4 = executor.submit(run_selenium1, f"-{ExperienceLevel}", f"{newSkills[1]}%2C+{newSkills[2]}", f"{undesired}", 1, resumeContent, locationpreference.replace(' ', '_'))
            #
